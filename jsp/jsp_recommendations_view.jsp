	...
	<body>
		...
		String sql = "SELECT count(*) FROM My_recommendations;"; 
		...
		String sqlStr = "select * from My_recommendations order by r_Num desc limit 3 offset ?;";
		...
		<table border = "0">
				<tr>
					<th>Article_Num</th>
					<td><%= rset.getString("r_num") %></td>
					<th style="color:#dc143c;">Title</th>
					<td ><%=rset.getString("r_title") %></td>
				</tr>
				<tr>
					<th>Write_Time</th>
					<td colspan="3"><%= rset.getString("r_time") %></td>
				</tr>
				<tr>
					<th>Content</th>
					<td colspan="3"><%= rset.getString("r_content") %></td>
				</tr>
		<%
	}
	%>
	...
	%>
	<a href="jsp_recommendations_view.jsp?p=<%=1%>" class="page"> [<<] </a>
	<a href="jsp_recommendations_view.jsp?p=<%=curPage-1 %>" class="page"> [<] </a>
	<%	}
	for( int i=startnum; i <= endnum; i++)
		{
			%>
				<a href ="jsp_recommendations_view.jsp?p=<%= i %>" class="page"><%= i %> </a>
			<% 	
		}
		if (curPage < totalPage) { %>

	<a href="jsp_recommendations_view.jsp?p=<%=curPage+1 %>" class="page"> [>] </a>
	<a href="jsp_recommendations_view.jsp?p=<%=totalPage%>" class="page"> [>>] </a>
	<%}
		%>
