	...
	<body>
		...
		String sql = "SELECT count(*) FROM My_Articles;"; 
		...
		String sqlStr = "select * from My_Articles order by a_ArticleNum desc limit 3 offset ?;";
		...
		<table border = "0">
				<tr>
					<th>Article_Num</th>
					<td><%= rset.getString("a_ArticleNum") %></td>
					<th style="color:#dc143c;">Title</th>
					<td ><%=rset.getString("a_ArticleName") %></td>
				</tr>
				<tr>
					<th>Write_Time</th>
					<td colspan="3"><%= rset.getString("a_Write_Time") %></td>
				</tr>
				<tr>
					<th>Eat_Time</th>
					<td colspan="3"><%= rset.getString("a_Eat_Time") %></td>
				</tr>
				<tr>
					<th>Eat_Place</th>
					<td colspan="3"><%= rset.getString("a_Eat_Place") %></td>
				</tr>
				<tr>
					<th>Meal_Time</th>
					<td colspan="3"><%= rset.getString("a_Meal_Time") %></td>
				</tr>
				<tr>
					<th>Food_Class</th>
					<td colspan="3"><%= rset.getString("a_Food_Class") %></td>
				</tr>
				<tr>
					<th>Content</th>
					<td colspan="3"><%= rset.getString("a_content") %></td>
				</tr>
		<%
	}
	%>
	...
	if(curPage > 1 )
		{
	%>
	<a href="jsp_board.jsp?p=<%=1%>" class="page"> [<<] </a>
	<a href="jsp_board.jsp?p=<%=curPage-1 %>" class="page"> [<] </a>
	<%	}
	for( int i=startnum; i <= endnum; i++)
		{
			%>
				<a href ="jsp_board.jsp?p=<%= i %>" class="page"><%= i %> </a>
			<% 	
		}
		if (curPage < totalPage) { %>

	<a href="jsp_board.jsp?p=<%=curPage+1 %>" class="page"> [>] </a>
	<a href="jsp_board.jsp?p=<%=totalPage%>" class="page"> [>>] </a>
	<%}
		%>
