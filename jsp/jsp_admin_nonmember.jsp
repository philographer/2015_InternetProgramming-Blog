	...
	<body>
		...
		String sqlStr = "select * from My_nonmember order by n_ArticleNum desc limit 3 offset ?;";
		...
		<table border = "0">
				<tr>
					<th>Article_Num</th>
					<td><%= rset.getString("n_ArticleNum") %></td>
					<th style="color:#dc143c;">Title</th>
					<td ><%=rset.getString("n_title") %></td>
				</tr>
				<tr>
					<th>Write_Time</th>
					<td colspan="3"><%= rset.getString("n_Write_Time") %></td>
				</tr>
				<tr>
					<th>Content</th>
					<td colspan="3"><%= rset.getString("n_content") %></td>
				</tr>
		<%
	}
	%>
	...
	%>
	<a href="jsp_admin_nonmember.jsp?p=<%=1%>" class="page"> [<<] </a>
	<a href="jsp_admin_nonmember.jsp?p=<%=curPage-1 %>" class="page"> [<] </a>
	<%	}
	for( int i=startnum; i <= endnum; i++)
		{
			%>
				<a href ="jsp_admin_nonmember.jsp?p=<%= i %>" class="page"><%= i %> </a>
			<% 	
		}
		if (curPage < totalPage) { %>

	<a href="jsp_admin_nonmember.jsp?p=<%=curPage+1 %>" class="page"> [>] </a>
	<a href="jsp_admin_nonmember.jsp?p=<%=totalPage%>" class="page"> [>>] </a>
	<%}
		%>
